...
object IOActors {
  ...
  def createForFile(fileName: Path,
                    ...
                    autoCloseAfter: Option[FiniteDuration] = Some(Duration(5, TimeUnit.SECONDS)))(implicit actorFactory: ActorRefFactory): ActorRef = {
...
class IOActor(private val fileHandleFactory:ExecutionContext=>FileIO,
              private val autoCloseAfter: Option[FiniteDuration] = Some(Duration(5, TimeUnit.SECONDS))) extends Actor {
  ...
  def receive = {
    ...
    case Read(start, amount) => {
      ...
      file.read(start, amount).andThen {
        case util.Success(bytes: ByteString) => currentSender ! ReadResponse(bytes, start, amount)
        case util.Success(unexpected) => currentSelf ! UnexpectedResult(unexpected)
        case util.Failure(ex) => currentSelf ! ExceptionOccured(ex)
      }
    }
    ...
    case Write(data, startPos) => {
      ...
      val currentSender = sender
      val currentSelf = self
      file.write(data, startPos).andThen {
        case util.Success(amountBytesWritten: Int) => currentSender ! WriteResponse(amountBytesWritten)
        case util.Success(unexpected) => currentSelf ! UnexpectedResult(unexpected)
        case util.Failure(ex) => currentSelf ! ExceptionOccured(ex)
      }
    }
  ...
  private case class ExceptionOccured(exception: Throwable)
